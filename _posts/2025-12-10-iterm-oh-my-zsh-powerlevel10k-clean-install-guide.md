...
date: 2025-12-10 17:30:00 +0900
categories: macOS
tags: [iTerm, zsh, Oh My Zsh, Powerlevel10k]
typora-root-url: ../

---
# ✈️ iTerm + Oh My Zsh + Powerlevel10k 설치 가이드

## 🎯 개요

macOS에서 터미널을 꾸미다 보면 이미 Oh My Zsh가 설치되어 있거나 예전에 시도하다 남은 설정 때문에 에러를 경험하는 경우가 있음
그래서 처음부터 순서대로 따라할 수 있게 문서로 정리
초기화부터 시작해서 깔끔하게 세팅하는 방법 안내
...
> 💡특이사항
>
> Oh My Zsh 설치 없이 Powerlevel10k만 설치하면 에러 발생
> 반드시 순서 지켜야 함
...
정상이라면 Powerlevel10k 설정 마법사가 자동 실행됨

---

## ✏️ verbose 옵션 이해

터미널에서 자주 보이는 `--verbose`는 영어로 "말이 많은, 장황한"에서 온 표현으로, "자세하게, 상세하게 출력해 달라"는 의미로 쓰이는 옵션임
...
즉, 터미널에서 --verbose를 보게 되면 "결과만 간단히"가 아니라 "과정과 디테일까지 보고 싶다"는 옵션이라고 이해하면 됨

> 💡특이사항
>
> 디버깅이나 문제 해결 시 verbose 옵션 유용함
> 일반 사용 시에는 굳이 켤 필요 없음
...
→ Powerlevel10k 재설치 필요

이 문서처럼 초기화부터 진행하면 대부분 발생하지 않음

> 💡특이사항
>
> 에러 발생 시 가장 빠른 해결책은 초기화 후 재설치
> 부분적인 수정보다 처음부터 다시 하는 게 시간 절약됨
...
터미널은 매일 쓰는 도구라 한 번만 제대로 세팅해두면 만족도 높음
