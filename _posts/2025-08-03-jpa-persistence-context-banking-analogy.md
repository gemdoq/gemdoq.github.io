...
date: 2025-08-03 13:08:38 +0900
categories: springboot
tags: [JPA, Persistence Context]
typora-root-url: ../

---
# ✈️  JPA 영속성 컨텍스트(Persistence Context)

## 🎯 개요

JPA의 핵심 개념인 영속성 컨텍스트를 은행 장부 비유로 쉽게 설명
DB와의 관계, 영속성 컨텍스트의 역할, 트랜잭션 커밋 시점의 변화 이해
...
영속성 컨텍스트는 `중앙은행 장부의 사본`이자, 변경 사항을 기록하는 `임시 장부` 역할. 개발 과정에서 데이터베이스에 직접 접근하는 대신, 영속성 컨텍스트를 통해 데이터를 관리
...
> 💡 왜 중요하나?
>
> JPA 애플리케이션의 성능과 데이터 일관성을 책임지는 핵심 메커니즘
> DB 접근 횟수를 줄이고, 변경 사항을 효율적으로 관리

---

## ✏️ 영속성 컨텍스트가 DB에 반영되는 순간
...
이는 JPA가 지향하는 개발의 편의성과 성능 최적화를 위한 핵심 메커니즘
